...
//const mapModel = require('../public/javascripts/map.js');
...
router.get('/map', function(req, res) {
    ...
    if (token != null) {
        request(`${apiAdr}/v1/city/${city}/bike?apiKey=${apiKey}`,
...
function serverUp() {
    request(`${apiAdr}/v1/city?apiKey=${apiKey}`, function (error, response) {
        if (!error && response.statusCode == 200) {
            apiAdr = "http://localhost:1337";
            console.log("Server is up");
        } else {
            apiAdr = "http://backend:1337";
