...
exports.Messages = {
    CLI: {
        ...
        DOCKERIZE_BUILD_COMMAND: 'Run the following commands to build the image and run the container:\n    cd %s\n    docker build -t %s .\n    docker run -d -p %d:%d %s',
        PROCESS_NAME_USED_ERROR: 'A process with the name "%s" is already running\nChange the environment\'s name in the data file or run start command with the "--pname" flag',
        PROCESS_START_LOG_ERROR: `Cannot start %s due to errors (see errors in ${(0, path_1.join)(config_1.Config.logsPath, '%s-error.log')})`,
        NO_RUNNING_PROCESS_FOUND: 'No process found with pid or name "%s"',
        NO_RUNNING_PROCESS: 'No process is running',
        MISSING_INDEX_OR_NAME_ERROR: '--index or --name is missing, you must provide one of them',
        DATA_TOO_OLD_ERROR: "These environment's data are too old or not a valid Mockoon environment.\nPlease verify or migrate them using a more recent version of the application",
        DATA_TOO_RECENT_ERROR: "These environment's data are too recent and cannot be run with the CLI\nPlease update the CLI with the following command 'npm install -g @mockoon/cli'",
        ENVIRONMENT_NOT_FOUND_INDEX_ERROR: 'Environment not found at index "%d"',
        ENVIRONMENT_NOT_FOUND_NAME_ERROR: 'Environment with name "%s" cannot be found',
